...
#setting up rocks
rockTimer = 0
rockRect = pygame.Rect(0, 0, 40, 40)
rockSurf = pygame.Surface((rockRect.w, rockRect.h))
rockRectList = []
...
while running:
    ...
    rand = random.random()
    if rockTimer % 90 == 0:
        newRockRect = rockRect.move(rand, 0)
        rockRectList.append(newRockRect)

    #draw rocks
    for j in rockRectList:
        rockRect.y += .5
        pygame.draw.rect(screen, (0,0,0), j)
    rockRect = rockRect.move(0,1)
    screen.blit(rockSurf, (rockRect.x, rockRect.y ))

...
while running:
    ...
    rock_timer = rock_timer - 1
    randx = random.randint(0,1240)
    if rock_timer == 0:
        rock_timer = 181
    if rock_timer == 180:
        newRockLoc = randx
        rockLocListX.append(newRockLoc)
        rockLocListY.append(-41)
        num = num + 1

    for location in rockLocListY:
        location = location + 3
        rockLocListY[num3] = location
        num3 = num3 + 1

    for num in range(len(rockLocListX)):
        screen.blit(rock_image, (rockLocListX[num], rockLocListY[num]))
        pygame.display.flip()

    #collisions with rocks
    for num in range(len(rockLocListX)):
        if boppiRect.collidepoint(rockLocListX[num],rockLocListY[num]) == True:
            lives = lives - 1
            boppiRect = pygame.Rect(0, 0, 45, 68)
            screen.blit(boppi, (0, 47))
            pygame.display.flip()

    #when rocks leave screen, deletes them from list
    for i in range(len(rockLocListX)):
        if ground.collidepoint(rockLocListX[num], rockLocListY[num]) == True:
            del rockLocListX[i]
            del rockLocListY[i]

    #make platforms diff color